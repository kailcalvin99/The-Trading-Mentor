...
- `trades` — id, pair, entry_time, risk_pct, liquidity_sweep, outcome, notes, behavior_tag, followed_time_rule, has_fvg_confirmation, stress_level, is_draft, ticker, side_direction, coach_feedback, setup_score, setup_type, created_at
...
- **Setup Quality Score**: `calculateSetupScore()` pure function scores trade setups 0–100 based on checklist completion (40pt), FVG presence (15pt), liquidity sweep (15pt), time rule (10pt), stress ≤5 (10pt), risk ≤1% (10pt). Live score badge shown in journal form (green ≥70, amber 50–69, red <50). Score saved to `setupScore` column in trades table. Displayed on trade cards.
- **Sit-Out Warning**: Modal warns trader when last 2 completed trades were losses OR last trade had stress ≥7. Appears when opening new trade form. Can dismiss or acknowledge sit-out. Both web and mobile.
